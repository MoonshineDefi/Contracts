...
// File contracts/farm/XxxVault.sol
...
contract XxxVault is Ownable, ReentrancyGuard {
    ...
    constructor(IXxxERC20 _xxx, uint256 _xxxPerBlock, address trustedForwarder) {
    ...
    function pendingXxx(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 xxxReward = multiplier
                .mul(xxxPerBlock)
                .mul(pool.allocPoint)
                .div(totalAllocPoint);
            accXxxPerShare = accXxxPerShare.add(
                xxxReward.mul(1e12).div(lpSupply)
            );
        }

        uint256 pending = user.amount.mul(accXxxPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    // Deposit LP tokens to XxxVault for Xxx allocation
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        require(
            block.number >= startBlock,
            "XxxVault: cannot deposit before farming start"
        );
    ...
    function payOrLockupPendingXxx(uint256 _pid) internal {
        ...
        uint256 pending = user.amount.mul(pool.accXxxPerShare).div(1e12).sub(
            user.rewardDebt
        );
    ...
    function safeXxxTransfer(address _to, uint256 _amount) internal {
        if (xxx.balanceOf(address(this)) > totalXxxInPools) {
            //XxxBal = total Xxx in XxxVault - total Xxx in Xxx pools, this will make sure that XxxVault never transfer rewards from deposited Xxx pools
            uint256 XxxBal = xxx.balanceOf(address(this)).sub(
                totalXxxInPools
            );
            if (_amount >= XxxBal) {
    ...
    function enableMetaTxns() public onlyOperator {
        require(
            !metaTxnsEnabled,
            "XxxVault: meta transactions are already enabled"
        );
    ...
    function disableMetaTxns() public onlyOperator {
        require(
            metaTxnsEnabled,
            "XxxVault: meta transactions are already disabled"
        );
